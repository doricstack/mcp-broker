...
`mcp-broker` is a local desktop broker for developers who run many MCP servers across MCP clients such as Codex, Claude, AGY, Cursor, and similar tools. It is not trying to be a hosted gateway, registry, container runtime, or enterprise policy plane.
...
- Codex, Claude, AGY, Cursor, or another MCP client loads too many tools at startup.
