...
RUNTIME_CONFIG_RELATIVE_PATH ?= config/broker.private.yaml
...
GOVERNANCE_APPROVAL ?=
FLEET_STATUS_TARGET ?=
FLEET_STATUS_AUTH_REF ?=
FLEET_STATUS_RETENTION_DAYS ?= 30
FLEET_STATUS_COLLECTOR_ID ?= local-fleet-collector
FLEET_STATUS_FILE ?= $(STATE_DIR)/broker-status.json
BREAK_GLASS_REASON ?=
...
PLUGIN_CLIENT ?= codex
PLUGIN_APPLY ?= 0
CONFIG_PRIVATE_PATH ?= $(RUNTIME_ROOT)/$(RUNTIME_CONFIG_RELATIVE_PATH)
CONFIG_PATH       ?= $(CONFIG_PRIVATE_PATH)
RUNTIME_CONFIG_MIGRATION_SOURCE ?= $(ROOT)/config/broker.private.yaml
RUNTIME_CONFIG_MIGRATION_APPLY ?= 0
LIVE_CONFIG_PATH  ?= $(CONFIG_PRIVATE_PATH)
...
RUNTIME_SYNC_WORKING_DIRECTORY ?= $(ROOT)
SYSTEMD_APPLY    ?= 0
...
RELEASE_MUTATION_TARGET ?= mutation-linux
...
RELEASE_GATE_PYTEST_MARKER_EXPRESSION ?= $(if $(strip $(PYTEST_MARKER_EXPRESSION)),$(PYTEST_MARKER_EXPRESSION),$(PUBLIC_RELEASE_PYTEST_MARKER_EXPRESSION))
...
PYTEST_TARGETED_LOG ?= $(TEST_LOG_DIR)/targeted.log
PYTEST_LIVE_TARGETED_LOG ?= $(TEST_LOG_DIR)/targeted-live.log
...
DOCKER_RELEASE_BUILDER ?= mcp-broker-release
DOCKER_RELEASE_BUILDER_DRIVER ?= docker-container
GITHUB_REPO ?= $(GITHUB_OWNER)/$(PACKAGE_SLUG)
GITHUB_REPOSITORY_URL ?= https://$(GITHUB_HOST)/$(GITHUB_REPO)
GITHUB_REPOSITORY_HOST_PATH ?= $(GITHUB_HOST)/$(GITHUB_REPO)
...
PRIVATE_GITHUB_REPOSITORY_URL ?= https://$(GITHUB_HOST)/$(PRIVATE_GITHUB_REPO)
...
SMITHERY_USER_AGENT ?= mcp-broker/$(PACKAGE_VERSION) (+$(GITHUB_REPOSITORY_URL))
...
MUTATE_FILE ?=
MUTATION_TESTS_TO_RUN ?=
MUTATION_PATH_SELECTOR ?= $(ROOT)/scripts/changed_mutation_paths.py
MUTATION_TEST_SELECTOR ?= $(ROOT)/scripts/select_affected_tests.py
MUTATION_TEST_TIER ?= push
...
MUTATION_RUN_LOG ?= $(QUALITY_DIR)/mutmut-run.log
...
export MCP_BROKER_LIVE_CONFIG_PATH := $(LIVE_CONFIG_PATH)
